...
namespace {
    ...
    int ConvSobel(const std::vector<int>& data, int idx, int width) {
        int gx = kernel_sobel_x[0][0] * data[idx - width - 1] + 
                kernel_sobel_x[0][1] * data[idx - width] +   
                kernel_sobel_x[0][2] * data[idx - width + 1] +
                
                kernel_sobel_x[1][0] * data[idx - 1] + 
                kernel_sobel_x[1][1] * data[idx] +          
                kernel_sobel_x[1][2] * data[idx + 1] +
                
                kernel_sobel_x[2][0] * data[idx + width - 1] + 
                kernel_sobel_x[2][1] * data[idx + width] +   
                kernel_sobel_x[2][2] * data[idx + width + 1];

        int gy = kernel_sobel_y[0][0] * data[idx - width - 1] + 
                kernel_sobel_y[0][1] * data[idx - width] +
                kernel_sobel_y[0][2] * data[idx - width + 1] +
                
                kernel_sobel_y[1][0] * data[idx - 1] + 
                kernel_sobel_y[1][1] * data[idx] +     
                kernel_sobel_y[1][2] * data[idx + 1] +
                
                kernel_sobel_y[2][0] * data[idx + width - 1] + 
                kernel_sobel_y[2][1] * data[idx + width] +
...
bool ChetverikovaESobelSEQ::ValidationImpl() {
    ...
    std::size_t expected_size = static_cast<std::size_t>(input.width) * 
